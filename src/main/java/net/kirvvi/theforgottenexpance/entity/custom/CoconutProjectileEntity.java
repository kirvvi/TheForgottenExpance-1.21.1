...
public class CoconutProjectileEntity extends AbstractArrow {

    public CoconutProjectileEntity(EntityType<? extends AbstractArrow> entityType, Level level) {
        super(entityType, level);
    }

    public CoconutProjectileEntity(LivingEntity shooter, Level level) {
        super(ModEntities.COCONUT.get(), shooter, level, new ItemStack(ModItems.COCONUT.get()), null);
    }
        ...
        if (!this.level().isClientSide) {
            ...
            if (isDropable) {
                ...
                if (this.level().getBlockState(placePos.below()).isAir()) {
                    ...
                    if (canPlaceBlock || hitFace == Direction.UP) {
                        Direction facing = hitFace.getAxis().isVertical() ? Direction.NORTH : hitFace.getOpposite();
                        ...
                        if (chance < 0.8) {
                            ...
                            this.level().setBlock(placePos, ModBlocks.COCONUT_BLOCK.get()
                                    .defaultBlockState()
                                    .setValue(BlockStateProperties.HORIZONTAL_FACING, facing), 3);
                        } else {
                        ...
                        this.spawnAtLocation(ModItems.COCONUT.get());
                    }
                    this.discard();
                }
            }
